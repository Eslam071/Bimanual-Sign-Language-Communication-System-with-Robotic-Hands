...
class MyNode:
    def __init__(self):
        ...
        rospy.Subscriber('/joint_states', JointState, self.my_callback)
    ...
    def my_callback(self, msg):
        ...
        self.thump = [msg.position[12] ,msg.position[17] ,msg.position[18]]   #Joints_NO: 3,8,9
        self.little = [msg.position[1] ,msg.position[2] ,msg.position[3]]     #Joints_NO: 10,11,12
        self.ring = [msg.position[4] ,msg.position[5] ,msg.position[6]]       #Joints_NO: 13,14,15
        self.middle = [msg.position[15] ,msg.position[7] ,msg.position[8]]    #Joints_NO: 6,16,17
        self.index = [msg.position[16] ,msg.position[9] ,msg.position[10]]    #Joints_NO: 7,18,19

        self.hand[0] = int(self.wrist * (110/90) )
        self.hand[1] = int( (sum_little - ((little[2]*np.cos(self.little[2])) + (little[1]*np.cos(self.little[1])) + (little[0]*np.cos(self.little[0])))) * (180/(np.pi*20)) * (75/180) * 1.7 )
        ...
        self.hand[3] = int( (sum_middle - ((middle[2]*np.cos(self.middle[2])) + (middle[1]*np.cos(self.middle[1])) + (middle[0]*np.cos(self.middle[0])))) * (180/(np.pi*20)) * (90/180) )
